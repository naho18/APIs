...
def find_afterparties():
    ...
    if location and distance and measurement:
        ...
        OAuth_token=os.environ['EVENTBRITE_TOKEN']
        App_key=os.environ['EVENTBRITE_KEY']

        payload = {'token' : OAuth_token, 'q': query, 'location.address': location,
        ...
        #create check to check if request was sucessful
        if r.ok:
            data = r.json()
